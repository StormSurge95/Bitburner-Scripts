let doc = eval("document"),
	...
	css = `<style id="scanalyzeCSS">
		.sc {white-space:pre; color:#ccc; font:20px monospace; line-height: 20px; }
...
export let main = ns => {
	...
	doc.head.insertAdjacentHTML('beforeend', doc.getElementById("scanalyzeCSS") ? "" : css);
	let s = ["home"],
		p = [""],
		r = { home: "home" },
		...
		fName = x => {
			...
			return `<a class="s${f.includes(x) ? " f" : ""}${ns.hasRootAccess(x) ? " r" : ""}">${x}</a>` +
				...
				`${' @'.repeat(ns.ls(x, ".cct").length)}` + `${' #'.repeat(ns.ls(x, ".lit").length)}`;
		};
	let tcommand = x => {
		tIn.value = x;
